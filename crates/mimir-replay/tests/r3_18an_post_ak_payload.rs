...
fn r3_18an_post_payload_poison_proves_zero_next_control_consumption() {
    ...
    let original = poisoned[10392 / 8] & 1 != 0;
...
fn r3_18an_all_47_frozen_am_rows_are_exact_and_stop_before_next_control() {
    ...
    for (index, (path, first_start, actor_object, expected_start, expected_end, expected_value)) in
        cases.into_iter().enumerate()
    {
        ...
        assert_eq!(
            got.following_payload.payload_start_bit, expected_start,
            "{path}"
        );
        assert_eq!(
            got.following_payload.payload_end_bit, expected_end,
            "{path}"
        );
