...
fn r3_16b_property_absent_stops_after_one_bit_without_lookup_or_payload() {
    ...
    let zero_tail =
        decode_replay_network_existing_actor_first_property_header_v1(&[0x00], 0, 47, &plan)
            .expect("property absent header");
    let one_tail =
        decode_replay_network_existing_actor_first_property_header_v1(&[0xfe], 0, 47, &plan)
...
fn r3_16b_property_present_uses_canonical_bounded_stream_and_resolves_inherited_property() {
    let plan = sample_001_lookup_plan();
    let header =
        decode_replay_network_existing_actor_first_property_header_v1(&[0x3d, 0x00], 0, 47, &plan)
...
fn r3_16b_payload_bits_are_opaque_and_do_not_change_header_result() {
    ...
    let payload_one =
        decode_replay_network_existing_actor_first_property_header_v1(&[0xfd, 0xff], 0, 47, &plan)
...
fn r3_16b_unresolved_stream_fails_closed() {
    ...
    assert!(
        actor_lookup
            .properties
            .iter()
            .any(|property| property.stream_id == 0)
    );
    actor_lookup
        .properties
        .retain(|property| property.stream_id != 0);

    let error =
        decode_replay_network_existing_actor_first_property_header_v1(&[0x01], 0, 47, &plan)
...
fn r3_16b_truncation_at_property_present_fails() {
    let plan = sample_001_lookup_plan();
    let error = decode_replay_network_existing_actor_first_property_header_v1(&[], 0, 47, &plan)
...
fn r3_16b_truncation_inside_bounded_stream_low_bits_fails() {
    let plan = sample_001_lookup_plan();
    let error =
        decode_replay_network_existing_actor_first_property_header_v1(&[0x10], 4, 47, &plan)
...
fn r3_16b_truncation_at_required_bounded_discriminator_fails() {
    let plan = sample_001_lookup_plan();
    let error =
        decode_replay_network_existing_actor_first_property_header_v1(&[0x04], 2, 47, &plan)
...
fn r3_16b_repeatability_is_exact() {
    let plan = sample_001_lookup_plan();
    let first =
        decode_replay_network_existing_actor_first_property_header_v1(&[0x3d, 0xaa], 0, 47, &plan)
            .expect("first decode");
    let second =
        decode_replay_network_existing_actor_first_property_header_v1(&[0x3d, 0xaa], 0, 47, &plan)
