...
fn av_expected_rows() -> Vec<AvExpected> {
    ...
    assert_eq!(
        lines.next(),
        Some(
            "label\tframe_index\tactor_ordinal\tactor_context_object_id\tfirst_start\tcontrol_start\tcontrol_end\tcontrol_value\tstream_start\tstream_end\tstream_id\tstream_id_bound\tprop_id_bits\tproperty_object_index\tattribute_tag\tpayload_start_bit\theader_stop_bit\tversion_major\tversion_minor\tnet_version\tis_rl_223"
        )
    );
...
fn r3_18av_published_au_matches_frozen_as_at_authority_exactly() {
    if std::env::var_os("R318AV_EXPECTED_TSV").is_none() {
        eprintln!(
            "R3_18AV_EXPECTED_TSV absent; runtime differential intentionally skipped outside evidence workflow"
        );
        return;
    }
        ...
        cases.into_iter().zip(expected.iter()).enumerate()
    {
        ...
        assert_eq!(
            exp.actor_context_object_id, actor_object,
            "actor object row {index}"
        );
        assert_eq!(exp.first_start, first_start, "first-start row {index}");
        assert_eq!(
            exp.control_start, control_start,
            "control-start row {index}"
        );
        ...
        let header = got
            .following_header
            .as_ref()
            .expect("true row following header");
        assert!(header.property_present, "{path}");
        assert_eq!(
            header.property_present_start_bit, exp.control_start,
            "{path}"
        );
        assert_eq!(header.property_present_end_bit, exp.control_end, "{path}");
        assert_eq!(
            header.actor_object_index, exp.actor_context_object_id,
            "{path}"
        );
        ...
        assert_eq!(
            header.resolved_property_object_index, exp.property_object_index,
            "{path}"
        );
        ...
        assert_eq!(
            got.stop_bit,
            exp.payload_start_bit.expect("payload start"),
            "{path}"
        );
