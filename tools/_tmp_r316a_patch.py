...
s = rep(s, "        updated_actors: &mut Vec<UpdatedAttribute>,\n        frame_index: usize,\n    ) -> Result<DecodedFrame, FrameError> {", "        updated_actors: &mut Vec<UpdatedAttribute>,\n        frame_index: usize,\n        r3_16a_selected: &mut bool,\n    ) -> Result<DecodedFrame, FrameError> {", "decode-frame-signature")
s = rep(s, "        let mut r3_15a_actor_ordinal = 0usize;\n        while bits", "        let r3_16a_total_bits = self.body.network_data.len() * 8;\n        let mut r3_15a_actor_ordinal = 0usize;\n        while bits", "total-bits")
s = rep(s, "                // new\n                if bits\n                    .read_bit()\n                    .ok_or(FrameError::NotEnoughDataFor(\"Is new actor\"))?\n                {", "                // new\n                let r3_16a_is_new = bits\n                    .read_bit()\n                    .ok_or(FrameError::NotEnoughDataFor(\"Is new actor\"))?;\n                let r3_16a_new_bit_end = r3_15a_offset(bits, r3_16a_total_bits);\n                if r3_16a_is_new {", "new-bit")
s = rep(s, "                    // While there are more attributes to update for our actor:\n                    while bits\n                        .read_bit()\n                        .ok_or(FrameError::NotEnoughDataFor(\"Is prop present\"))?\n                    {", "                    // While there are more attributes to update for our actor:\n                    loop {\n                        let r3_16a_property_present_start =\n                            r3_15a_offset(bits, r3_16a_total_bits);\n                        let r3_16a_property_present = bits\n                            .read_bit()\n                            .ok_or(FrameError::NotEnoughDataFor(\"Is prop present\"))?;\n                        let r3_16a_property_present_end =\n                            r3_15a_offset(bits, r3_16a_total_bits);\n                        if !r3_16a_property_present { break; }", "property-present")
s = rep(s, "                        bits.refill_lookahead();\n                        if bits.lookahead_bits() < cache_info.prop_id_bits + 1 {", "                        let r3_16a_stream_start = r3_15a_offset(bits, r3_16a_total_bits);\n                        bits.refill_lookahead();\n                        if bits.lookahead_bits() < cache_info.prop_id_bits + 1 {", "stream-start")
s = rep(s, "                        let stream_id = StreamId(stream_id_raw as i32);\n\n                        // Look the stream id up", "                        let stream_id = StreamId(stream_id_raw as i32);\n                        let r3_16a_stream_end = r3_15a_offset(bits, r3_16a_total_bits);\n\n                        // Look the stream id up", "stream-end")
...
insert = '''                        let r3_16a_payload_start = r3_15a_offset(bits, r3_16a_total_bits);
                        if !*r3_16a_selected {
                            ...
                            let actor_name = self.body.objects.get(actor_index).map(String::as_str).unwrap_or("<actor-oob>").replace('\t', "_");
                            let property_name = self.body.objects.get(property_index).map(String::as_str).unwrap_or("<property-oob>").replace('\t', "_");
...
s = rep(s, "        let mut buf = [0u8; 1024];\n\n        while !bits.is_empty()", "        let mut buf = [0u8; 1024];\n        let mut r3_16a_selected = false;\n\n        while !bits.is_empty()", "selected-state")
s = rep(s, "                    &mut updated_actors,\n                    frames.len(),\n                )", "                    &mut updated_actors,\n                    frames.len(),\n                    &mut r3_16a_selected,\n                )", "decode-call")
