...
fn uid_parts(value: &mimir_replay::ReplayNetworkUniqueIdV1) -> (&'static str, u64) {
    ...
    match &value.remote_id {
        ...
        ReplayNetworkUniqueIdRemoteV1::PlayStation { name, unknown, online_id } => {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for line in request.lines().filter(|x| !x.trim().is_empty()) {
        ...
        if f.len() != 18 { return Err(format!("expected 18 fields got {}", f.len()).into()); }
        let label=f[0];
        ...
        let aa=decode_replay_network_existing_actor_after_first_primitive_second_property_payload_following_payload_control_following_header_v1(
            &network,&prior,&plan,ctx)?;
        if aa.stop_bit != payload_start || aa.following_header.payload_start_bit != Some(payload_start) {
            return Err(format!("{label}: AA payload start drift").into());
        }
        let tag=aa.following_header.resolved_attribute_tag.ok_or("missing AA tag")?;
        if format!("{tag:?}") != tag_name { return Err(format!("{label}: tag drift").into()); }

        let (payload_end,width,active,actor,intv,uid_system,uid_local,uid_remote,uid_fp,result_key) =
            match tag {
                ReplayNetworkAttributeTagV1::ActiveActor => {
                    ...
                    let (a,id)=match &got.value {
                        ...
                        other => return Err(format!("{label}: unexpected active value {other:?}").into()),
                    };
                    (got.payload_end_bit,u64::from(got.payload_width),if a{"1"}else{"0"}.to_owned(),id.to_string(),"na".to_owned(),"na".to_owned(),"na".to_owned(),"na".to_owned(),"na".to_owned(),format!("{:?}",got.value))
                }
                ReplayNetworkAttributeTagV1::Int => {
                    let got=decode_replay_network_primitive_scalar_v1(&network,payload_start,tag)?;
                    let v=match got.value { ReplayNetworkPrimitiveScalarValueV1::Int(v)=>v, ref other=>return Err(format!("{label}: unexpected int {other:?}").into()) };
                    (got.payload_end_bit,u64::from(got.payload_width),"na".to_owned(),"na".to_owned(),v.to_string(),"na".to_owned(),"na".to_owned(),"na".to_owned(),"na".to_owned(),format!("{:?}",got.value))
                }
                ReplayNetworkAttributeTagV1::UniqueId => {
                    let got=decode_replay_network_k2_v1(&network,payload_start,tag,k2ctx())?;
                    let uid=match &got.value { ReplayNetworkK2ValueV1::UniqueId(v)=>v, other=>return Err(format!("{label}: unexpected uid {other:?}").into()) };
                    let (rk,fp)=uid_parts(uid);
                    if rk=="Other" { return Err(format!("{label}: unsupported observed uid system {}",uid.system_id).into()); }
                    (got.payload_end_bit,u64::from(got.payload_width),"na".to_owned(),"na".to_owned(),"na".to_owned(),uid.system_id.to_string(),uid.local_id.to_string(),rk.to_owned(),format!("{fp:016x}"),format!("{:?}",got.value))
                }
                other => return Err(format!("{label}: unsupported AC tag {other:?}").into()),
            };

        let repeated_key = match tag {
            ReplayNetworkAttributeTagV1::Int => format!("{:?}",decode_replay_network_primitive_scalar_v1(&network,payload_start,tag)?.value),
            _ => format!("{:?}",decode_replay_network_k2_v1(&network,payload_start,tag,k2ctx())?.value),
        };
        let repeatability = repeated_key == result_key;
        if repeatability { repeat += 1; }

        let trunc_len=usize::try_from(payload_start/8)?.min(network.len());
        let truncation=match tag {
            ReplayNetworkAttributeTagV1::Int => decode_replay_network_primitive_scalar_v1(&network[..trunc_len],payload_start,tag).is_err(),
            _ => decode_replay_network_k2_v1(&network[..trunc_len],payload_start,tag,k2ctx()).is_err(),
        };
        if truncation { trunc += 1; }

        let wrong_tag_negative=match tag {
            ReplayNetworkAttributeTagV1::Int => {
                decode_replay_network_k2_v1(&network,payload_start,ReplayNetworkAttributeTagV1::ActiveActor,k2ctx())
                    .map(|x| x.payload_end_bit != payload_end || format!("{:?}",x.value)!=result_key).unwrap_or(true)
            }
            _ => {
                decode_replay_network_primitive_scalar_v1(&network,payload_start,ReplayNetworkAttributeTagV1::Int)
                    .map(|x| x.payload_end_bit != payload_end || format!("{:?}",x.value)!=result_key).unwrap_or(true)
            }
        };
        if wrong_tag_negative { wrong_tag += 1; }

        let wrong_context_negative=if tag==ReplayNetworkAttributeTagV1::Int {
            true
        } else {
            decode_replay_network_k2_v1(
                &network,payload_start,tag,
                ReplayNetworkK2DecodeContextV1{net_version:9,is_rl_223:false}
            ).is_err()
        };
        if wrong_context_negative { wrong_ctx += 1; }

        let mut poisoned=network.clone();
        for off in 0..16u64 {
            set_bit(&mut poisoned,payload_end+off,off%2==0).map_err(std::io::Error::other)?;
        }
        let poisoned_key=match tag {
            ReplayNetworkAttributeTagV1::Int => format!("{:?}",decode_replay_network_primitive_scalar_v1(&poisoned,payload_start,tag)?.value),
            _ => format!("{:?}",decode_replay_network_k2_v1(&poisoned,payload_start,tag,k2ctx())?.value),
        };
        let poison_invariance=poisoned_key==result_key;
        if poison_invariance { poison += 1; }
        ...
        println!(
            ...
            u8::from(repeatability),u8::from(truncation),u8::from(wrong_tag_negative),
            u8::from(wrong_context_negative),u8::from(poison_invariance),
        );
    }
    if rows!=47 || repeat!=47 || trunc!=47 || wrong_tag!=47 || wrong_ctx!=47 || poison!=47 {
        return Err(format!("aggregate fail rows={rows} repeat={repeat} trunc={trunc} wrong_tag={wrong_tag} wrong_ctx={wrong_ctx} poison={poison}").into());
    }
    println!("R3_18AC_NATIVE_AGG\trows=47\trepeatability=47\ttruncation=47\twrong_tag_negative=47\twrong_context_negative=47\tpost_payload_poison=47\tanother_control_bits_consumed=0");
    Ok(())
}
