...
fn all_admitted_tags_decode_at_aligned_and_unaligned_offsets() {
    let cases = [
        ...
        (ReplayNetworkAttributeTagV1::Float, u64::from(1.5f32.to_bits())),
        (ReplayNetworkAttributeTagV1::Int, u64::from((-12345i32) as u32)),
...
fn byte_and_enum_extremes_are_exact() {
    for raw in [0u64, 255] {
        ...
        .unwrap();
        assert_eq!(decoded.value, ReplayNetworkPrimitiveScalarValueV1::Byte(raw as u8));
    }
    ...
    for raw in [0u64, 2047] {
        ...
        .unwrap();
        assert_eq!(decoded.value, ReplayNetworkPrimitiveScalarValueV1::Enum(raw as u16));
    }
}
...
fn float_preserves_raw_ieee_identity_including_nan_and_signed_zero() {
    ...
    for raw_bits in patterns {
        ...
        .unwrap();
        match decoded.value {
            ReplayNetworkPrimitiveScalarValueV1::Float { raw_bits: raw, value } => {
...
fn int_preserves_twos_complement_boundaries() {
    for value in [i32::MIN, -1, 0, i32::MAX] {
        ...
        .unwrap();
        assert_eq!(decoded.value, ReplayNetworkPrimitiveScalarValueV1::Int(value));
    }
}
...
fn int64_preserves_twos_complement_boundaries() {
    for value in [i64::MIN, -1, 0, i64::MAX] {
        ...
        .unwrap();
        assert_eq!(decoded.value, ReplayNetworkPrimitiveScalarValueV1::Int64(value));
    }
}
...
fn invalid_start_positions_fail_closed() {
    let bytes = [0u8];
    assert!(decode_replay_network_primitive_scalar_v1(
        &bytes,
        8,
        ReplayNetworkAttributeTagV1::Boolean,
    )
...
fn poison_bits_after_scalar_are_not_consumed() {
    ...
    let clean_result = decode_replay_network_primitive_scalar_v1(&clean, start as u64, tag).unwrap();
    let poisoned_result =
