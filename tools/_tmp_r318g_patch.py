...
TEST = Path('crates/mimir-replay/tests/r3_18g_second_property_header.rs')

text = LIB.read_text(encoding='utf-8')
anchor = 'fn network_existing_actor_after_first_property_control_error(\n'
if text.count(anchor) != 1:
    raise SystemExit(f'expected one R3.18D error-helper anchor, found {text.count(anchor)}')
if 'R3.18G BEGIN bounded second-property header composition' in text:
    raise SystemExit('R3.18G block already present')

block = r'''// R3.18G BEGIN bounded second-property header composition
        ...
        Some(tag) => {
            return Err(
                network_existing_actor_after_first_primitive_second_property_header_error(
                    ...
                    format!(
                        "R3.18G admits only Int/String second-header contexts, got {tag:?}"
                    ),
                ),
            );
        }
...
fn aligned_and_unaligned_second_header_starts_preserve_exact_coordinates() {
    for (first_start, expected_second_start) in [(2usize, 40u64), (3usize, 41u64)] {
        ...
        assert_eq!(decoded.control.property_present_start_bit, expected_second_start);
