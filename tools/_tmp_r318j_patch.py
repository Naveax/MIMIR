...
TEST = Path('crates/mimir-replay/tests/r3_18j_second_property_payload.rs')

marker = '/// R3.18J bounded second-property payload value.'
text = SOURCE.read_text(encoding='utf-8')
if marker in text:
    raise SystemExit('R3.18J source marker already present')
...
pub fn decode_replay_network_existing_actor_after_first_primitive_second_property_payload_v1(
    ...
    if second_header.stop_bit != payload_start_bit || header_composition.stop_bit != payload_start_bit
    {
    ...
    let second_payload = match tag {
        ReplayNetworkAttributeTagV1::Int => {
            ...
            if decoded.payload_start_bit != payload_start_bit || decoded.stop_bit != decoded.payload_end_bit {
                return Err(network_bit_error(
    ...
    let stop_bit = match &second_payload {
        ...
        ReplayNetworkExistingActorSecondPropertyPayloadV1::String(decoded) => decoded.payload_end_bit,
    };
...
'''
SOURCE.write_text(text.rstrip() + addition + '\n', encoding='utf-8')
...
fn retag(plan: &mut ReplayNetworkLookupPlanV1, actor: usize, stream: u32, tag: ReplayNetworkAttributeTagV1) {
...
fn int_packet(first_start: usize, next_present: bool, second_raw: u32, trailing_bits: usize) -> Vec<u8> {
    ...
    for first_start in [0usize, 3usize, 7usize] {
        ...
        match decoded.second_payload.as_ref().unwrap() {
            ReplayNetworkExistingActorSecondPropertyPayloadV1::Int(value) => {
                ...
                assert_eq!(value.value, ReplayNetworkPrimitiveScalarValueV1::Int(-12345));
            }
...
fn string_wrong_context_fails_closed_before_semantic_widening() {
    ...
    let first = first_property(&bytes, 0, 47, &plan);
    retag(&mut plan, 47, 30, ReplayNetworkAttributeTagV1::String);
    let error = decode_replay_network_existing_actor_after_first_primitive_second_property_payload_v1(
        &bytes,
        &first,
        &plan,
        ctx(10, true),
    ...
    .unwrap_err();
    assert!(error.to_string().contains("unsupported-second-string-context"));
}
...
fn int_truncation_rejects_and_post_payload_poison_is_invariant() {
    ...
    let expected = decode_replay_network_existing_actor_after_first_primitive_second_property_payload_v1(
        &clean,
        &first,
        &plan,
        ctx(10, false),
    ...
    assert!(decode_replay_network_existing_actor_after_first_primitive_second_property_payload_v1(
        &truncated,
        &first,
        &plan,
        ctx(10, false),
    ...
    let got = decode_replay_network_existing_actor_after_first_primitive_second_property_payload_v1(
        &poisoned,
        &first,
        &plan,
        ctx(10, false),
...
fn string_truncation_and_malformed_length_reject_atomically() {
    ...
    write_bits(&mut malformed, payload_start, 32, u64::from(i32::MIN as u32));
    let error = decode_replay_network_existing_actor_after_first_primitive_second_property_payload_v1(
        &malformed,
        &first,
        &plan,
...
fn result_is_repeatable_and_tag_outside_int_string_stays_closed() {
    ...
    let one = decode_replay_network_existing_actor_after_first_primitive_second_property_payload_v1(
        &bytes,
        &first,
        &plan,
        ctx(10, false),
    ...
    .unwrap();
    let two = decode_replay_network_existing_actor_after_first_primitive_second_property_payload_v1(
        &bytes,
        &first,
        &plan,
        ctx(10, false),
