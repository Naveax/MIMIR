...
marker = "fn r3_18o_label() -> String {\n"
helper = r'''fn r3_18o_target_usize(name: &str) -> usize {
    std::env::var(name)
        .unwrap_or_else(|_| panic!("R3.18O missing target env {name}"))
        .parse::<usize>()
        .unwrap_or_else(|_| panic!("R3.18O invalid usize target env {name}"))
}

fn r3_18o_target_i32(name: &str) -> i32 {
    std::env::var(name)
        .unwrap_or_else(|_| panic!("R3.18O missing target env {name}"))
        .parse::<i32>()
        .unwrap_or_else(|_| panic!("R3.18O invalid i32 target env {name}"))
}

'''
s = rep(s, marker, helper + marker, "target helpers")

old = '''                        let r3_18o_payload_start_bit =
                            r3_18o_offset(bits, r3_18o_total_bits);
                        let attribute = attr_decoder.decode(attr.attribute, bits, buf).map_err(
'''
new = r'''                        let r3_18o_payload_start_bit =
                            r3_18o_offset(bits, r3_18o_total_bits);
                        ...
                        {
                            println!(
                                "R3_18O_HEADER\tlabel={}\tframe_index={}\tactor_ordinal={}\tactor_id={}\tactor_context_object_id={}\tproperty_ordinal=2\tproperty_present_start_bit={}\tproperty_present_end_bit={}\tstream_id_start_bit={}\tstream_id_end_bit={}\tstream_id={}\tstream_id_bound={}\tprop_id_bits={}\tproperty_object_id={}\tattribute_tag={:?}\tversion_major={}\tversion_minor={}\tnet_version={}\tpayload_start_bit={}",
                                r3_18o_label(),
                                frame_index,
                                r3_18o_current_actor_ordinal,
                                actor_id.0,
                                object_id.0,
                                r3_18o_property_present_start_bit,
                                r3_18o_property_present_end_bit,
                                r3_18o_stream_id_start_bit,
                                r3_18o_stream_id_end_bit,
                                stream_id.0,
                        ...
                        let attribute = attr_decoder.decode(attr.attribute, bits, buf).map_err(
'''
s = rep(s, old, new, "following-header emission before payload decode")
frame.write_text(s, encoding="utf-8", newline="\n")
print("R3_18O_BOXCARS_FOLLOWING_HEADER_PATCH=PASS")
