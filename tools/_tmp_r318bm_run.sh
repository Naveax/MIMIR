...
test -n "$REAL_CARGO"
WRAP_DIR="$RUNNER_TEMP/r318bm-cargo-wrapper"
rm -rf "$WRAP_DIR"
mkdir -p "$WRAP_DIR"
cat > "$WRAP_DIR/cargo" <<'SH'
#!/usr/bin/env bash
set -euo pipefail
if [ -f Cargo.toml ] && grep -Fqx 'name = "boxcars"' Cargo.toml; then
  marker="${RUNNER_TEMP}/r318bm_boxcars_bench_devdeps_stripped"
  python3 - "$marker" <<'PY'
from pathlib import Path
import sys
...
if counts == [1, 1]:
    ...
    marker.write_text('criterion=0.8\ngungraun=0.19.0\n', encoding='utf-8', newline='\n')
    print('R3_18BM_BOXCARS_BENCH_DEVDEPS_STRIPPED=PASS criterion=0.8 gungraun=0.19.0')
elif counts == [0, 0] and marker.is_file():
    pass
else:
    raise SystemExit(f'R3.18BM Boxcars benchmark dev-dependency authority mismatch counts={counts}')
PY
fi
exec "$R3_18BM_REAL_CARGO" "$@"
SH
chmod +x "$WRAP_DIR/cargo"
export R3_18BM_REAL_CARGO="$REAL_CARGO"
export PATH="$WRAP_DIR:$PATH"
test "$(command -v cargo)" = "$WRAP_DIR/cargo"
echo R3_18BM_CARGO_WRAPPER=PASS
chmod +x "$DIR/_tmp_r318bm_inner.sh"
bash -n "$DIR/_tmp_r318bm_inner.sh"
python3 -m py_compile "$DIR/r318bm_boxcars_patch.py"
