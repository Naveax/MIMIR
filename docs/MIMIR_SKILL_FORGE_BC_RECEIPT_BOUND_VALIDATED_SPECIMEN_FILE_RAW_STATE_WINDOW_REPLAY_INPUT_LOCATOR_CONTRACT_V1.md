# MIMIR Skill Forge BC Receipt-Bound Validated Specimen-File Raw-State-Window Replay-Input Locator Contract v1
...
- the exact admitted boundary above:
  ...
  - `LowBoostRecoveryBcReceiptBoundReplayInputLocatorReopenDecisionResultV1`
- the exact per-specimen admitted contract inputs preserved from the truthful non-located
  replay-input-access/source-binding realization result
- the exact replay-input locator boundary input tuple
- the exact replay-input locator output-boundary shape
- the exact bounded contract-definition output, disposition, notes, and failure rules

### Why it exists

The replay-input locator reopen-decision boundary already fixed:
...
tries to realize even a first replay-input locator result.

### How it differs from the replay-input locator reopen-decision boundary below it

- The reopen-decision boundary answers whether replay-input locator work must be reopened at all.
...
This replay-input locator contract remains family-specific because:
...
- no second family exists yet to justify a shared replay-input locator contract framework
...
- each `consumed_replay_input_access_source_binding_boundary_input`
- each `preserved_replay_input_access_source_binding_output_boundary`
- each `replay_input_access_source_binding_disposition`
- `realization_disposition`
- `realization_notes`
- `chosen_replay_input_access_contract_shape`

From `LowBoostRecoveryBcReceiptBoundReplayInputLocatorReopenDecisionResultV1`, this pass consumes
exactly:
...
replay-input-access/source-binding realization result, the replay-input locator reopen decision,
...
- defining one honest future replay-input locator-facing handle contract for one admitted truthful
  non-located replay-input-access/source-binding specimen
...
- one new replay-input locator boundary input tuple
- one new replay-input locator output-boundary shape

This contract is not allowed to implement:
...
- `admitted_replay_input_access_source_binding_boundary_input:
  LowBoostRecoveryBcReceiptBoundValidatedSpecimenFileRawStateWindowReplayInputAccessSourceBindingBoundaryInputV1`
- `admitted_replay_input_access_source_binding_output_boundary:
  LowBoostRecoveryBcReceiptBoundValidatedSpecimenFileRawStateWindowReplayInputAccessSourceBindingOutputBoundaryV1`

Those preserved admitted inputs remain the only truthful non-located replay-input-access/source-
binding specimen evidence this pass is allowed to use.

This pass does not recompute those values from lower layers.

### Exact replay-input locator boundary input tuple

For one admitted specimen, the replay-input locator boundary input is:
...
This input tuple is copied only from the admitted truthful non-located replay-input-access/source-
binding boundary input.

### Exact replay-input locator output-boundary shape

For one admitted specimen, the replay-input locator output boundary is:
...
- for one admitted truthful non-located replay-input-access/source-binding specimen, the tuple
...
- that contract must keep the admitted replay-input-access/source-binding output boundary aligned
  with the new replay-input locator output boundary on the full tuple above
...
- preserved from the admitted truthful non-located replay-input-access/source-binding boundary
- fixed to
...
- the admitted replay-input-access/source-binding output boundary still carries only a contract-only
  future parser-consumable replay handle
- the new replay-input locator output boundary also carries only a contract-only future
...
- chosen `chosen_replay_input_locator_contract_shape`

Each lane result is:

- `LowBoostRecoveryBcReceiptBoundValidatedSpecimenFileRawStateWindowReplayInputLocatorContractLaneResultV1`
...
- `LowBoostRecoveryBcReceiptBoundValidatedSpecimenFileRawStateWindowReplayInputLocatorContractSpecimenResultV1`
...
- `ReplayInputAccessSourceBindingRealizationBoundaryPreserved`
- `ReplayInputLocatorReopenDecisionBoundaryPreserved`
...
- `ReceiptBoundSourceLineagePreservedForReplayInputLocatorContractOnly`
...
- `ReplayInputLocatorContractOnlyBoundaryDefined`
...
The truthful non-located replay-input-access/source-binding realization result plus replay-input
locator reopen-decision result may enter this contract-definition boundary only when all of the
following hold:

1. the inputs are exactly:
   - `LowBoostRecoveryBcReceiptBoundValidatedSpecimenFileRawStateWindowReplayInputAccessSourceBindingRealizationResultV1`
   - `LowBoostRecoveryBcReceiptBoundReplayInputLocatorReopenDecisionResultV1`
2. the realization result still satisfies the exact replay-input locator reopen-decision admission
   rules
3. `source_realization_disposition ==
   RealizedForTruthfulNonLocatedReplayInputAccessSourceBindingOnly`
4. `source_realization_notes` still equal the exact truthful non-located replay-input-access/
   source-binding realization note set
...
9. `decision_disposition ==
   ReopenJustifiedForReceiptBoundReplayInputLocatorContract`
10. `decision_notes` still equal the exact replay-input locator reopen-decision note set
11. `chosen_replay_input_locator_contract_shape == ReceiptBoundReplayInputLocatorOnly`
12. `specimen_count > 0`
13. `group_count > 0`
14. realization and reopen counts match exactly
15. realization and reopen audited family root directory references match exactly
16. the audited family root directory still ends in `low_boost_recovery_bc_v1`
17. the audited family root directory still exists and is a directory at contract-definition time
18. per-specimen lane/specimen order, artifact ids, anchored specimen-file paths, source lineage,
    deferred observation lineage, and admitted handle kind still satisfy the exact realization
    validator
19. no lower boundary is silently reopened to repair or reinterpret the admitted realization input

Admission here means only:

- this contract surface may be defined from trusted truthful non-located replay-input-access/
  source-binding evidence plus the trusted replay-input locator reopen decision
...
- degraded replay-input locator reopen-decision input
...
- one machine-verifiable replay-input locator contract exists above the truthful non-located
  replay-input-access/source-binding realization result
- one admitted truthful non-located specimen now contributes one exact locator boundary input tuple
- one admitted truthful non-located specimen now contributes one exact locator output-boundary
  shape
...
- a first actual replay-input locator realization pass above this contract, still without locator
  implementation
